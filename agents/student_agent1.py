...
class StudentAgent1(Agent):
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        self.winning_square = self.get_winning_square(
 ...
 # Squares near borders are worth less points than squares closer to the center
    ...
    def heuristic_function(self, chess_board, my_pos, adv_pos, max_step):
        ...
        move_score = self.heuristic_score_move(
            chess_board, my_pos, adv_pos, max_step)*7
        ...
        if self.winning_square != None:
            myDistance = (
                1/(1+self.get_distance(my_pos, self.winning_square))) * 200
        else:
            myDistance = 0

        return round(move_score + score_walls + score_position + myDistance, 3)
...
# -----------------------------------------------------------------------------------------------------------------
    ...
    def get_winning_square(self, chess_board, my_pos, adv_pos):
        for row in range(chess_board.shape[0]):
            for col in range(chess_board.shape[0]):

                if not chess_board[row, col, 1]:
                    chess_board[row, col, 1] = True
                    win, my_score, adv_score = self.check_endgame(
                        chess_board, (row, col), adv_pos)
                    if win:
                        chess_board[row, col, 1] = False
                        return (row, col)
                    chess_board[row, col, 1] = False

                if not chess_board[row, col, 2]:
                    chess_board[row, col, 2] = True
                    win, my_score, adv_score = self.check_endgame(
                        chess_board, (row, col), adv_pos)
                    if win:
                        chess_board[row, col, 2] = False
                        return (row, col)
                    chess_board[row, col, 2] = False

        return None

    def get_distance(self, me, winner):
        x1, y1 = me
        x2, y2 = winner
